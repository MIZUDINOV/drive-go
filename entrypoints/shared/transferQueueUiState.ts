...
export async function getTransferPopoverSeenUpTo(): Promise<number> {
  return new Promise((resolve) => {
    browser.storage.local.get(
      [STORAGE_KEY_TRANSFER_POPOVER_SEEN_UP_TO],
      (result) => {
        const normalized = normalizeTimestamp(
          result[STORAGE_KEY_TRANSFER_POPOVER_SEEN_UP_TO],
        );
        if (normalized !== null) {
          resolve(normalized);
          return;
        ...
        browser.storage.local.set(
          { [STORAGE_KEY_TRANSFER_POPOVER_SEEN_UP_TO]: now },
          () => {
            resolve(now);
          },
        );
...
export async function setTransferPopoverSeenUpTo(
  timestamp: number,
): Promise<void> {
