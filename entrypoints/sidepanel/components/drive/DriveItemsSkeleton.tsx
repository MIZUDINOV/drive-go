...
import {
  DriveViewMode,
  type DriveViewMode as DriveViewModeType,
} from "./driveTypes";
...
function GridFileSkeletonItem() {
  return (
    <div class="drive-item drive-item-grid skeleton-item-grid">
      ...
      <Skeleton
        class="skeleton skeleton-preview"
        radius={8}
    ...
    <div class="drive-item drive-item-grid-folder skeleton-item-grid-folder">
      <div class="drive-grid-tile-top">
        <div class="drive-grid-title-wrap">
          ...
          <Skeleton
            class="skeleton skeleton-title-folder"
            height={13}
            radius={6}
            style={{ flex: "1 1 0", "min-width": "0" }}
          >
  ...
  if (props.viewMode === DriveViewMode.List) {
    return (
      <div class="drive-items-list" aria-busy="true">
        <For each={Array(8)}>{() => <ListSkeletonItem />}</For>
      </div>
    );
  }
  ...
  return (
    <div class="drive-grid-layout" aria-busy="true">
      <div class="drive-grid-folders-row">
        <For each={Array(2)}>{() => <GridFolderSkeletonItem />}</For>
      </div>
      <div class="drive-items-grid">
        <For each={Array(6)}>{() => <GridFileSkeletonItem />}</For>
      </div>
    </div>
  );
}
